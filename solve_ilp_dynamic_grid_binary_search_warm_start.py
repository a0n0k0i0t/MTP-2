...
def build_model(points, input_box_indices, qbox_bounds, Wr, Wb, epsilon, d=2, delta_base=2, use_dynamic_grid=True, warm_start_box=None):
    n = len(points)
    ...
    Z = {}
    M = max(len(vals) for vals in sorted_coords) # Used max instead of 2*max from dinkelbach for binary search
...
def solve_fair_range_binary_search_warm_start(points, input_box_indices, qbox_bounds, alpha, beta, Wr, Wb, epsilon, d=2, delta_base=2, use_dynamic_grid=True, start_low=0.0, warm_start_box=None):
    """
    Performs binary search on gamma with dynamic grid optimization.
    Uses warm-starting by maintaining the Gurobi model across iterations AND between phases.
    ...
    model, S_ino, S_io, S_oi, l_vars, r_vars, sorted_coords, allowed_L_vars, allowed_R_vars = build_model(
        points, input_box_indices, qbox_bounds, Wr, Wb, epsilon, d, delta_base, use_dynamic_grid, warm_start_box)
    ...
    while high - low > 0.005: 
        ...
        # Formulate new LHS and add new constraint
        ...
        # Inject MIP Start if we have a best_box (clear old hints first)
...
if __name__ == "__main__":
    ...
    print(f"Solving ILP with Dynamic Grid Binary Search Warm Start (alpha={alpha}, beta={beta}, epsilon={epsilon}, delta={delta_base})...")
    
    start_time = time.time()
    try:
        print("--- PHASE 1: DYNAMIC GRID ---")
        # Find an initial approx with the dynamic grid
        gamma_approx, box_approx = solve_fair_range_binary_search_warm_start(points, input_indices, qbox, alpha, beta, Wr, Wb, epsilon, d=d, delta_base=delta_base, use_dynamic_grid=True)
        
        print("\n--- PHASE 2: EXACT SEARCH WARM START ---")
        if box_approx is not None:
            print(f"Using Phase 1 result (Gamma: {gamma_approx:.4f}) as warm start boundary for Exact Search.")
            # Start the true low where the approx left off, inject the best box
            gamma, box = solve_fair_range_binary_search_warm_start(points, input_indices, qbox, alpha, beta, Wr, Wb, epsilon, d=d, delta_base=1, use_dynamic_grid=False, start_low=gamma_approx, warm_start_box=box_approx)
        else:
            print("Phase 1 failed. Running exact full space from scratch...")
            gamma, box = solve_fair_range_binary_search_warm_start(points, input_indices, qbox, alpha, beta, Wr, Wb, epsilon, d=d, delta_base=1, use_dynamic_grid=False)
    except:
        traceback.print_exc()
        gamma = 0
        box = None
