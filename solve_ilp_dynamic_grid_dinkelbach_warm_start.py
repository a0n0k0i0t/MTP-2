...
def solve_fair_range_Dinkleback_warm_start(points, input_box_indices, qbox_bounds, alpha, beta, Wr, Wb, epsilon, d=2, delta_base=2, use_dynamic_grid=True, start_gamma=0.0, warm_start_box=None):
    """
    Performs Dinkleback search on gamma.
...
if __name__ == "__main__":
    ...
    print(f"Solving ILP with Dynamic Grid Dinkleback Warm Start (alpha={alpha}, beta={beta}, epsilon={epsilon}, delta={delta_base})...")
    
    start_time = time.time()
    try:
        print("--- PHASE 1: DYNAMIC GRID ---")
        gamma_approx, box_approx = solve_fair_range_Dinkleback_warm_start(points, input_indices, qbox, alpha, beta, Wr, Wb, epsilon, d=d, delta_base=delta_base, use_dynamic_grid=True)
        ...
        if box_approx is not None:
            ...
            gamma, box = solve_fair_range_Dinkleback_warm_start(points, input_indices, qbox, alpha, beta, Wr, Wb, epsilon, d=d, delta_base=1, use_dynamic_grid=False, start_gamma=gamma_approx, warm_start_box=box_approx)
        else:
            print("Phase 1 failed. Running exact full space from scratch...")
            gamma, box = solve_fair_range_Dinkleback_warm_start(points, input_indices, qbox, alpha, beta, Wr, Wb, epsilon, d=d, delta_base=1, use_dynamic_grid=False)
    except:
        traceback.print_exc()
        gamma = 0
        box = None
