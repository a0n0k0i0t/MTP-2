...
std::vector<DatasetPoint> read_custom_dataset(const std::string &filename) {
  ...
  if (!file_handler.is_open()) {
    ...
    return dataset; // Return empty vector on failure
  }
  ...
  while (std::getline(file_handler, line)) {
    ...
    std::string age_str, sex_str;

    // Assuming comma-separated values: age,sex
    if (std::getline(ss, age_str, ',') && std::getline(ss, sex_str, ',')) {
      try {
        double age = std::stod(age_str);
        int sex = std::stoi(sex_str);

        // Map sex to color: 1 (Female) -> blue, 2 (Male) -> red
        std::string color_str = (sex == 1) ? "blue" : "red";

        // ID will be set sequentially after sorting
        dataset.emplace_back(age, color_str, 0);
      } catch (...) {
  ...
  // The algorithm requires data to be sorted in ascending order by value (age)
...
// --- Modified Calculation: Tversky Index ---
...
// *** MODIFIED: Now takes vector<double> to support weighted accumulated scores
// ***
void find_best_fair_range_brute_force(
  ...
  // Since weights are baked into the totals, we just look for a 1:1 balance of
  // scores
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i; j < n; ++j) {
      // Calculate weighted score instead of raw counts
      double red_score =
...
int main() {
  ...
  std::string filename = "../dataset/sorted_age_sex.csv";
  ...
  // *** MODIFIED: Define explicit weights ***
  const double WEIGHT_RED = 1.0;
  const double WEIGHT_BLUE = 1.0;

  // *** MODIFIED: Track weights as doubles instead of plain integer counts ***
  ...
  // *** MODIFIED: The tree logic uses the pre-weighted values from the arrays
  // ***
  ...
  // --- Find Best Fair Range using Optimized 3D Range Tree ---
  ...
  // --- Find Best Fair Range using Brute Force for Verification ---
