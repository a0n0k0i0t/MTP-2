...
// *** MODIFIED: Custom parser for synthetic_30k_unique (Value -> Category) ***
std::vector<DatasetPoint> read_custom_dataset(const std::string &filename) {
  ...
  while (std::getline(file_handler, line)) {
    ...
    // Convert any commas to spaces to handle both CSV and space-separated
    ...
    if (ss >> value >> category) {
      // *** MODIFIED: If category is 1 put blue, otherwise red ***
...
int main() {
  ...
  // *** TARGET THE SYNTHETIC 30K DATASET ***
  std::string filename = "../dataset/synthetic_30k_unique.csv";
  ...
  // Define explicit weights (Adjust these if you want different weighting)
  ...
  // *** MODIFIED: Bounds reflecting the values in the synthetic dataset ***
  double start_val = 10000.0;
  double end_val = 30000.0;
