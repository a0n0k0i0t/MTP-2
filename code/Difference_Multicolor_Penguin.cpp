...
unique_ptr<Result> queryBestBruteForce(const Data &data, int epsilon,
  ...
  // 1. Naive linear scan for bounds (Unoptimized: replaces binary search)
  int L0 = 0;
  while (L0 < n && data.keys[L0] < min(startVal, endVal)) {
    L0++;
  }

  int R0 = n - 1;
  while (R0 >= 0 && data.keys[R0] > max(startVal, endVal)) {
    R0--;
  }
  ...
  // 2. Pure O(N^2) loops checking every single combination
  for (int L = 1; L <= n; L++) {
    for (int R = L; R <= n; R++) {

      bool fair = true;

      // 3. Unoptimized dimension check (No 'break' early exit)
      for (int d = 0; d < data.m; d++) {
        double diff = abs(data.points[R][d] - data.points[L - 1][d]);
        if (diff > epsilon) {
          fair = false;
          // Notice: We don't break out of the loop here anymore.
          // It will keep checking remaining dimensions pointlessly.
        }
      }
...
Data readCSV(const string &filename, int expectedColors) {
  ...
  while (getline(br, line)) {
    ...
    if (parts.size() < 2)
      continue; // Adapted for Penguin format

    try {
      double key = stod(parts[0]);
      string raceStr = parts[1]; // Adapted: Species is now index 1
