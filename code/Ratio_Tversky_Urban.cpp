...
std::vector<DatasetPoint> read_custom_dataset(const std::string &filename) {
  ...
  while (std::getline(file_handler, line)) {
    ...
    if (iss >> range_val >> mid_val >> category) {
      std::string color_str = (category == 2) ? "blue" : "red";
      dataset.emplace_back(range_val, color_str, 0);
    }
  }
...
void find_best_fair_range_brute_force(
  ...
  // Since weights are baked into the totals, we just look for a 1:1 balance
  // again
...
int main() {
  ...
  std::vector<DatasetPoint> dataset =
      read_custom_dataset("../dataset/UrbanDB.csv");
  if (dataset.empty()) {
  ...
  double start_val = -0.130983;
  double end_val = 0.228074;
  ...
  // *** MODIFIED: Because weights are baked into the cumulative arrays,
  // the tree logic reverts back to the standard 1+eps boundary check. ***
