  ...
  for (int L = 1; L <= n; L++) {
    for (int R = L; R <= n; R++) {
      ...
      for (int d = 0; d < data.m; d++) {
        ...
        if (diff > epsilon) {
          fair = false;
          // REMOVED OPTIMIZATION: We no longer "break" here.
          // The loop continues checking all dimensions even if it's already
          // unfair.
        }
      }

      // REMOVED OPTIMIZATION: We now compute Tversky similarity for EVERY
      // range, simulating the heavy math workload regardless of the "fair"
      // status.
      double t = tversky(L, R, L0 + 1, R0 + 1, alpha, beta);

      // Only record it if it was actually a fair range
      if (fair) {
        if (t > bestT) {
          bestT = t;
          bestL = L;
