...
// *** MODIFIED: Custom parser for Texas Tribune (Category -> Value) ***
std::vector<DatasetPoint> read_custom_dataset(const std::string &filename) {
  ...
  while (std::getline(file_handler, line)) {
    ...
    // Convert any commas to spaces to handle both CSV and space-separated lines
    ...
    // Read: [Category] [Value]
    if (ss >> category >> value) {
      // Map category 1 to "blue", 0 to "red"
...
int main() {
  ...
  // *** TARGET THE TEXAS TRIBUNE DATASET ***
  std::string filename = "../dataset/sorted_texas_tribune.csv";
  ...
  // *** MODIFIED: Bounds reflecting the salary values in Texas Tribune ***
  double start_val = 10000.0;
  double end_val = 20000.0;
